# -*- encoding: utf-8 -*-
# @Time: 2022/03/24 23:09
# @Author: librah
# @Description: Live video streaming player
# @File: player.py
# @Version: 1.0
...
class Live_Player(object):
    ...
    def fetch(self, quality, next_chunk_set, seg_idx, chunk_idx, take_action, num_chunk, playing_speed):
        """
        Description: Fetch chunks from server
        Args: 
            quality: the ABR's selected quality
            next_chunk_set: The chunks set to be downloaded
            seg_idx: current segment's index
            chunk_idx: the last downloaded chunk's index
            take_action: handle rtt
            num_chunk: the number of chunks to be downloaded
            playing_speed: playing speed of the player
        Return:
            chunk_sent: the chunks that have sent
            downloading_fraction: the downloading time
            freezing_fraction: the freezing time
            time_out: 1(freezing time exceeds the freezing_tol), 0(normal)
            start_state: initial state
            rtt: the round trip time of network
        """
        # Action initialization
        start_state = self.state
        chunk_size = next_chunk_set[quality] # in Kbits not KBits
        chunk_start_time = seg_idx * self.seg_duration + chunk_idx * self.chunk_duration # the start time of this chunk
        ...
        if take_action:
            ...
            if self.state == 1:
                # modified based on playing speed, adjusted, * speed
                self.playing_time += np.minimum(self.buffer, playing_speed*rtt)
                # modified based on playing speed, real time, /speed
                freezing_fraction += np.maximum(rtt-self.buffer/playing_speed, 0.0)
                # modified based on playing speed, adjusted, * speed
                self.buffer = np.maximum(0.0, self.buffer - playing_speed*rtt)
        ...
        while True:
            ...
            duration = self.time_trace[self.time_idx] * Env_Config.ms_in_s - self.last_trace_time # in ms
            # compute the deliverable size in duration
            ...
            if deliverable_size + chunk_sent > chunk_size:
                ...
                if self.state == 1:
                    ...
                    # modified based on playing speed
                    temp_freezing = np.maximum(fraction - self.buffer/playing_speed, 0.0)
                    # freezing time exceeds the freezing_tol, will not fetch chunk
                    ...
                    freezing_fraction += np.maximum(fraction - self.buffer/playing_speed, 0.0)
                    self.playing_time += np.minimum(self.buffer, playing_speed*fraction)
                    self.buffer = np.maximum(self.buffer - playing_speed*fraction, 0.0)

                    if np.round(self.playing_time + self.buffer, 2) == np.round(chunk_start_time, 2):
                        # Satisfy the download conditions
            ...
            if self.state == 1:
                assert freezing_fraction == 0.0
                temp_freezing = np.maximum(duration - self.buffer/playing_speed, 0.0)
                self.playing_time += np.minimum(self.buffer, playing_speed*duration)
                ...
                self.buffer = np.maximum(self.buffer - playing_speed*duration, 0.0)
    ...
    def sync_palying_timeout(self, sync_time):
        """
        Description: Synchronized the player's time
        Args: 
            sync_time: set the playing_time
        Return:
            None
        """
        self.buffer = 0.0
    ...
    def playing_time_back(self, index_gap):
        ...
        if not np.round(self.playing_time, 1) % self.seg_duration == 0.0:
            print("playing_time_back: {}".format(self.playing_time))
    ...
    def skip_with_time(self, jump_time, encoder_head_time):
        ...
        if np.round(self.buffer, 2) >= np.round(jump_time, 2):
            ...
            print("in player, buffer: {}, jump_time: {}, playing_time: {}".format(self.buffer, jump_time, self.playing_time))
            assert 0 == 1
        return
    
    def repeat(self):
        """
        Description: Repeat playing some last segments
        Args: 
            None
        Return:
            None
    ...
    def adjust_start_up_ssh(self, new_start_up_ssh):
        """
        Description: Adjust the start up ssh
        Args: 
            new_start_up_ssh: the new value
        Return:
            None
        """
    ...
    def wait(self, wait_time, playing_speed  = 1.0):
        """
        Description: Get the freezing time
        Args: 
            wait_time: wait time for playing
            playing_speed: playing speed
        Return:
    ...
    def check_sync(self, server_time):
        # sync = 0
    ...
    def throughput_trace_amplifyer_mean(self, trace):
        """
        Description: Amplify the throughput trace
        Args: 
            trace: throughput trace
        Return:
            new_trace: the new throughput trace that amplifyed
        """
    ...
    def reset(self, throughput_trace, time_trace, trace_name, testing=False, bw_amplify=False):
        """
            Description: Reset the live player
            Args: 
                throughput_trace: throughput trace list
                time_trace； time trace list
                trace_name； the name of trace
                testing: True/False
                bw_amplify: True/False
            Return:
                None
        """
        ...
        if testing:
            ...
            self.time_idx = self.myRandom.randint(1, len(self.time_trace)-1)
        self.last_trace_time = self.time_trace[self.time_idx-1] * Env_Config.ms_in_s
        self.buffer = 0.0 
        self.state = 0
